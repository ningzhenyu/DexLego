...
class ArtMethod FINAL {
 public:
  ...
  void LogMethod(const char* caller, const void * entry, const DexFile::CodeItem* code) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);

  void SetShouldManipulate();

  bool ShouldManipulate() SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
  ...
  void* GetHookTrampoline();

  EntryHookInfo* GetHookInfo() const;
  ...
  void addHookReal(size_t pointer_size);
 ...
 private:
  ...
  ALWAYS_INLINE T GetEntryPoint(MemberOffset offset, size_t pointer_size) const {
    ...
    if (offset.Uint32Value() == EntryPointFromInterpreterOffset(pointer_size).Uint32Value()) {
      EntryHookInfo* hook = GetHookInfo();
      if (nullptr != hook) {
        // char tmp[50];
        // sprintf(tmp, "GetEntryPoint: %d %p", dex_method_index_, hook);
        // LOG(ERROR) << tmp;
        addr = reinterpret_cast<uintptr_t>(hook) + 12;
        // sprintf(tmp, "addr: %" PRIXPTR, addr);
        // LOG(ERROR) << tmp;
  ...
  ALWAYS_INLINE void SetEntryPoint(MemberOffset offset, T new_value, size_t pointer_size) {
    ...
    if (offset.Uint32Value() == EntryPointFromInterpreterOffset(pointer_size).Uint32Value()) {
      ...
      if (nullptr != hook) {
        // char tmp[50];
        // sprintf(tmp, "SetEntryPoint: %d %p", dex_method_index_, hook);
        // LOG(ERROR) << tmp;
        addr = reinterpret_cast<uintptr_t>(hook) + 12;
        // sprintf(tmp, "addr: %" PRIXPTR, addr);
        // LOG(ERROR) << tmp;
