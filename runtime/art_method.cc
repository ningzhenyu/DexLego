...
bool ArtMethod::ShouldManipulate() {
  const EntryHookInfo* info = GetHookInfo();
  // const EntryHookInfo* info = nullptr;
//  if (dex_method_index_ <= 35) {
//    LOG(ERROR) << "ShouldManipulate";
//    const DexFile* dex_file = GetDexFile();
//    LOG(ERROR) << "got dex file";
//    if (dex_file) {
//      if (nullptr != info) {
//        char tmp[30];
//        sprintf(tmp, "%p", info);
//        LOG(ERROR) << "ShouldManipulate " << tmp << " " << dex_method_index_
//            << " " << dex_file->GetMethodName(dex_file->GetMethodId(dex_method_index_));
//      } else {
//        LOG(ERROR) << "ShouldManipulate " << dex_method_index_  << " null "
//            << dex_file->GetMethodName(dex_file->GetMethodId(dex_method_index_));
//      }
//    } else {
//      if (nullptr != info) {
//        char tmp[30];
//        sprintf(tmp, "%p", info);
//        LOG(ERROR) << "ShouldManipulate " << info << " " << dex_method_index_;
//      } else {
//        LOG(ERROR) << "ShouldManipulate null " << dex_method_index_;
//      }
//    }
//  }
  if (nullptr != info) {
...
EntryHookInfo* ArtMethod::GetHookInfo() const {
  size_t pointer_size = sizeof(void*);
//  if (dex_method_index_ <= 35) {
//    LOG(ERROR) << "GetHookInfo";
//  }
  // void* entry = GetEntryPointFromQuickCompiledCodePtrSizeWithoutCheck(pointer_size);
  const auto addr = reinterpret_cast<uintptr_t>(this) + EntryPointFromInterpreterOffset(pointer_size).Uint32Value();
  EntryPointFromInterpreter* entry;
  if (pointer_size == sizeof(uint32_t)) {
    entry = reinterpret_cast<EntryPointFromInterpreter*>(*reinterpret_cast<const uint32_t*>(addr));
  } else {
    auto v = *reinterpret_cast<const uint64_t*>(addr);
    DCHECK_EQ(reinterpret_cast<uint64_t>(reinterpret_cast<EntryPointFromInterpreter*>(v)), v) << "Conversion lost bits";
    entry = reinterpret_cast<EntryPointFromInterpreter*>(v);
  }
...
//    if (nullptr != entry) {
//      char tmp[30];
//      sprintf(tmp, "entry: %p", entry);
//      LOG(ERROR) << tmp;
//    } else {
//      LOG(ERROR) << "empty entry";
//    }
//  }

  EntryHookInfo* hook = reinterpret_cast<EntryHookInfo*>(entry);
//  if (dex_method_index_ <= 35) {
//    if (nullptr != hook) {
//      char tmp[30];
//      sprintf(tmp, "hook: %p", hook);
//      LOG(ERROR) << tmp;
//    } else {
//      LOG(ERROR) << "empty hook";
//    }
//  }
  if (nullptr == hook || strncmp(hook->magic, "droidreveal", strlen("droidreveal")) != 0) {
//    if (dex_method_index_ <= 35) {
//      LOG(ERROR) << "not hooked ";
//    }
    return nullptr;
  }
  return hook;
...
void ArtMethod::addHookReal(size_t pointer_size) {
  const auto addr = reinterpret_cast<uintptr_t>(this) + EntryPointFromInterpreterOffset(pointer_size).Uint32Value();
  EntryPointFromInterpreter* ori_interpreter_entry;
  if (pointer_size == sizeof(uint32_t)) {
    ori_interpreter_entry = reinterpret_cast<EntryPointFromInterpreter*>(*reinterpret_cast<const uint32_t*>(addr));
  } else {
    auto v = *reinterpret_cast<const uint64_t*>(addr);
    DCHECK_EQ(reinterpret_cast<uint64_t>(reinterpret_cast<EntryPointFromInterpreter*>(v)), v) << "Conversion lost bits";
    ori_interpreter_entry = reinterpret_cast<EntryPointFromInterpreter*>(v);
  }

  EntryHookInfo* info = reinterpret_cast<EntryHookInfo*>(malloc(sizeof(EntryHookInfo)));
  info->ori_interpreter_entry = ori_interpreter_entry;
  strncpy(info->magic, "droidreveal", strlen("droidreveal"));
  // SetEntryPointFromQuickCompiledCodePtrSizeWithoutCheck(info, pointer_size);
  if (pointer_size == sizeof(uint32_t)) {
    uintptr_t ptr = reinterpret_cast<uintptr_t>(info);
    DCHECK_EQ(static_cast<uint32_t>(ptr), ptr) << "Conversion lost bits";
    *reinterpret_cast<uint32_t*>(addr) = static_cast<uint32_t>(ptr);
  } else {
    *reinterpret_cast<uint64_t*>(addr) = reinterpret_cast<uintptr_t>(info);
  }
}

void ArtMethod::SetShouldManipulate() {
  EntryHookInfo* info = GetHookInfo();
  if (info == nullptr) {
    addHookReal(sizeof(void*));
  }
}
...
void dumpmemory(const void *ptr) {
  if (ptr) {
  LOG(ERROR) << "dumpmemory start" << ptr;
  const uint32_t* temp = reinterpret_cast<const uint32_t*>(ptr);
  uint32_t lineIndex = 0;
  while (lineIndex < 50) {
    LOG(ERROR) << StringPrintf("line %2d: 0x%x 0x%x 0x%x 0x%x", lineIndex, *(temp + lineIndex * 4), *(temp + 1 + lineIndex * 4), *(temp + 2 + lineIndex * 4), *(temp + 3 + lineIndex * 4));
    ++lineIndex;
  }
  LOG(ERROR) << "dumpmemory end ";
  } else {
  LOG(ERROR) << "method code empty";
  }
}

/* void dumpCodeItem(std::string location, uint32_t index, const DexFile::CodeItem* code);
void dumpCodeItem(std::string location, uint32_t index, const DexFile::CodeItem* code) {
  char* path = new char[100];
  std::hash<std::string> hash;
  sprintf(path, "/data/data/com.zhenyu.dl/code_%zu.dat", hash(location.c_str()));
  LOG(ERROR) << "try dump code item " << index << " " << location << " " << path;
  FILE* dump_file = fopen(path, "ab+");
  if (dump_file) {
    fwrite(&index, sizeof(uint32_t), 1, dump_file);
    size_t size = sizeof(uint32_t) * 2 + sizeof(uint16_t) * (4 + code->insns_size_in_code_units_);
    fwrite(&size, sizeof(size), 1, dump_file);
    fwrite(&code->registers_size_, sizeof(uint16_t), 1, dump_file);
    fwrite(&code->ins_size_, sizeof(uint16_t), 1, dump_file);
    fwrite(&code->outs_size_, sizeof(uint16_t), 1, dump_file);
    fwrite(&code->tries_size_, sizeof(uint16_t), 1, dump_file);
    fwrite(&code->debug_info_off_, sizeof(uint32_t), 1, dump_file);
    fwrite(&code->insns_size_in_code_units_, sizeof(uint32_t), 1, dump_file);
    for (uint32_t i = 0; i < code->insns_size_in_code_units_; ++i) {
      fwrite(&(code->insns_[i]), sizeof(uint16_t), 1, dump_file);
    }
    fflush(dump_file);
    fclose(dump_file);
  } else {
...
void ArtMethod::LogMethod(const char* caller, const void* entry, const DexFile::CodeItem* code) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) {
  EntryHookInfo* info = GetHookInfo();
  if (info /* && dex_method_index_>= 14082 && dex_method_index_ <= 35 */) {
  //   Class* declaringClass = GetDeclaringClass();
  //   if (declaringClass && (declaringClass->IsLoaded() || declaringClass->IsErroneous())) {
  //   DexCache* cache = declaringClass->GetDexCache();
  //   if (cache) {
  //     const DexFile* dexFile = cache->GetDexFile();
  //     if (dexFile && dexFile->GetLocation().find("zhenyu") != std::string::npos) {
       LOG(ERROR) << caller << ":" << dex_method_index_ << " " << entry << " " << code;
      // LOG(ERROR) << "ArtMethod::LogMethod " << dex_method_index_;
  //     }
  //   }
  //   }
  //     if (nullptr != code) {
        // dumpCodeItem(GetDexFile()->GetLocation(), dex_method_index_, code);
        // Dumper::Instance()->DumpCode(GetDexFile()->GetLocation(), dex_method_index_, code);
  //     }
  }
}
