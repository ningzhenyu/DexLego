...
// bool dump = false;
static std::map<pid_t, bool> pid_map;
...
bool shouldDump2() {
  ...
  char buff[1024];
  pid_t pid = getpid();
  std::map<pid_t, bool>::iterator it = pid_map.find(pid);
  if (it != pid_map.end()) {
    return it->second;
  }
      ...
      // if (!strncmp(buff, "Name:", 5)) {
        ...
        if (filter.fail()) {
          if (strstr(buff, "zhenyu") || strstr(buff, "ecspride") || strstr(buff, "mit")
              ...
              || strstr(buff, "wayne")) {
            LOG(ERROR) << "default filter in classlinker";
            pid_map.insert(std::pair<pid_t, bool>(pid, true));
            fclose(fp);
            return true;
          }
        } else {
          LOG(ERROR) << "file filter in classlinker";
  ...
  pid_map.insert(std::pair<pid_t, bool>(pid, false));
  return false;
}

void ClassLinker::LinkCode(ArtMethod* method, const OatFile::OatClass* oat_class,
                           uint32_t class_def_method_index, bool dump) {
  Runtime* const runtime = Runtime::Current();
  if (runtime->IsAotCompiler()) {
  ...
  if (manipulate) {
    method->SetShouldManipulate();
    const EntryHookInfo* info = method->GetHookInfo();
    if (nullptr == info) {
      LOG(FATAL) << "hook failed";
    } else {
      // LOG(ERROR) << "hook succeed";
    }
  }
}
...
void ClassLinker::LoadClass(Thread* self, const DexFile& dex_file,
  ...
  const uint8_t* class_data = dex_file.GetClassData(dex_class_def);
  const char* descriptor = dex_file.GetClassDescriptor(dex_class_def);
  // std::string desString(descriptor);
  // dump = desString.find("zhenyu") != std::string::npos && desString.find("MainActivity") != std::string::npos;
  bool dump = shouldDump2();
  // dump = Dumper::Instance()->shouldDump();
  // if (desString.find("zhenyu") != std::string::npos) {
  std::string location = dex_file.GetLocation();
  dump = dump && location.compare(0, 8, "/system/") != 0;
  // ClassDex dump_clz;
  if (dump) {
    // dump = !strstr(descriptor, "qihoo") && !strstr(descriptor, "ali.mob")
    //     && !strstr(descriptor, "aaaaaaaa") && !strstr(descriptor, "baidu") && !strstr(descriptor, "tencent");
    dump = !Dumper::Instance()->shouldFilterClass(descriptor);
    // LOG(ERROR) << "ClassLinker::LoadClass " << descriptor << " " << location << " " << dump;
  }
  ...
  if (Runtime::Current()->IsStarted() && !Runtime::Current()->IsAotCompiler()) {
    OatFile::OatClass oat_class = FindOatClass(dex_file, klass->GetDexClassDefIndex(),
                                               &has_oat_class);
    if (has_oat_class) {
      LoadClassMembers(self, dex_file, class_data, klass, &oat_class, dump);
    }
  }
  if (!has_oat_class) {
    LoadClassMembers(self, dex_file, class_data, klass, nullptr, dump);
  }

  if (dump) {
    Dumper::Instance()->DumpClassFromDex(dex_file, dex_class_def);
    // LOG(ERROR) << "ClassLinker::LoadClass end";
  }
}
...
void ClassLinker::LoadClassMembers(Thread* self, const DexFile& dex_file,
                                   ...
                                   Handle<mirror::Class> klass,
                                   const OatFile::OatClass* oat_class, bool dump) {
  {
    ...
    for (size_t i = 0; it.HasNextStaticField(); i++, it.Next()) {
      ...
      if (dump) {
        Dumper::Instance()->DumpEncodedFieldFromDex(dex_file, sfields[i].GetDexFieldIndex(), STATIC, sfields[i].GetAccessFlags());
      }
    }
    ...
    for (size_t i = 0; it.HasNextInstanceField(); i++, it.Next()) {
      ...
      if (dump) {
        Dumper::Instance()->DumpEncodedFieldFromDex(dex_file, ifields[i].GetDexFieldIndex(), INSTANCE, ifields[i].GetAccessFlags());
      }
    }
    ...
    for (size_t i = 0; it.HasNextDirectMethod(); i++, it.Next()) {
      ...
      LinkCode(method, oat_class, class_def_method_index, dump);
      if (dump) {
        Dumper::Instance()->DumpEncodedMethodFromDex(dex_file, method->GetDexMethodIndex(), DIRECT, method->GetAccessFlags());
      }
    ...
    for (size_t i = 0; it.HasNextVirtualMethod(); i++, it.Next()) {
      ...
      LinkCode(method, oat_class, class_def_method_index, dump);
      if (dump) {
        Dumper::Instance()->DumpEncodedMethodFromDex(dex_file, method->GetDexMethodIndex(), VIRTUAL, method->GetAccessFlags());
      }
      class_def_method_index++;
    }
    DCHECK(!it.HasNext());
...
bool ClassLinker::InitializeClass(Thread* self, Handle<mirror::Class> klass,
  ...
  if (num_static_fields > 0) {
    ...
    std::string location = dex_file.GetLocation();
    // bool shouldDump = Dumper::Instance()->shouldDump() && location.compare(0, 8, "/system/") != 0;
    bool shouldDump = shouldDump2() && location.compare(0, 8, "/system/") != 0;
    if (shouldDump) {
      std::string store;
      const char* descriptor = klass->GetDescriptor(&store);
      // if (!strstr(descriptor, "qihoo") && !strstr(descriptor, "ali.mob")
      //   && !strstr(descriptor, "aaaaaaaa") && !strstr(descriptor, "baidu") && !strstr(descriptor, "tencent")) {
      if (!Dumper::Instance()->shouldFilterClass(descriptor)) {
        Dumper::Instance()->DumpStaticValuesFromDex(dex_file, *dex_class_def);
      }
    }
    ...
    if (value_it.HasNext()) {
      ...
      for ( ; value_it.HasNext(); value_it.Next(), field_it.Next()) {
        ...
        if (shouldDump) {
          Dumper::Instance()->DumpEncodedFieldFromDex(dex_file, field->GetDexFieldIndex(), STATIC, field->GetAccessFlags());
        }
