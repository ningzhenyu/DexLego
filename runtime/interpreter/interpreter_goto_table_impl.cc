...
#define HANDLE_PENDING_EXCEPTION(_offset)                                   \
  do {                                                                      \
    force_execution_offset = _offset;                                       \
    if (IGNORE_EXCEPTION) {                                       \
      self->ClearException();                                       \
      ADVANCE(force_execution_offset);                                       \
    } else {                                       \
      goto exception_pending_label;                                           \
    }                                       \
  } while (false)                                                           \
...
#define POSSIBLY_HANDLE_PENDING_EXCEPTION(_is_exception_pending, _offset)   \
  do {                                                                      \
    if (UNLIKELY(_is_exception_pending)) {                                \
      HANDLE_PENDING_EXCEPTION(_offset);                                  \
    } else {                                                              \
      ADVANCE(_offset);                                                   \
    }                                                                     \
  } while (false)
...
#undef INSTRUCTION_HANDLER
  ...
  ALLOW_TEMP_MEMORY("ExecuteGotoImpl");
  ...
  HANDLE_INSTRUCTION_START(PACKED_SWITCH) {
    ...
    offset = force_ret == 0 ? offset : force_ret;
    if (IsBackwardBranch(offset)) {
      BACKWARD_BRANCH_INSTRUMENTATION(offset);
      if (UNLIKELY(self->TestAllFlags())) {
        self->CheckSuspend();
...
#pragma clang diagnostic pop
  ...
  HANDLE_INSTRUCTION_START(IF_EQ) {
    ...
    if ((shadow_frame.GetVReg(inst->VRegA_22t(inst_data)) == shadow_frame.GetVReg(inst->VRegB_22t(inst_data))
        && force_ret == 0) || (force_ret != 2 && force_ret != 0)) {
