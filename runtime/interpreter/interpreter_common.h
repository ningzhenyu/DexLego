...
static inline bool DoInvoke(Thread* self, ShadowFrame& shadow_frame, const Instruction* inst,
                            ...
                            const uint16_t* inst_list, JValue* result) {
  ...
  HANDLE_INSTRUCTION_ABOUT_INVOKE();
...
static inline bool DoInvokeVirtualQuick(Thread* self, ShadowFrame& shadow_frame,
  ...
  HANDLE_INSTRUCTION_ABOUT_INVOKE_QUICK();
  if (UNLIKELY(called_method == nullptr)) {
    CHECK(self->IsExceptionPending());
    result->SetJ(0);
    return false;
...
bool DoIGetQuick(ShadowFrame& shadow_frame, const Instruction* inst, uint16_t inst_data,
    MapAndList*& map_and_list, const uint16_t* inst_list) SHARED_LOCKS_REQUIRED(Locks::mutator_lock_);
